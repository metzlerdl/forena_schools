-- DROP FUNCTION a_profile_student_scores(bigint, integer, integer)
...
DECLARE 
  ...
  v_max_school_year INTEGER; 
BEGIN
  SELECT COALESCE(p_school_year, i_school_year()) + school_year_offset, COALESCE(p_school_year, i_school_year()) 
  INTO v_school_year, v_max_school_year
    ...
    LEFT JOIN 
      (select row_number() OVER (partition by ts.person_id, test_id, ts.seq order by date_taken desc) r,
        ...
        WHERE ts.person_id=p_person_id
          AND ts.school_year BETWEEN v_school_year AND v_max_school_year) a 
