...
select 
  ...
  bss.seq, 
  t.name as test_name, 
  t.abbrev as test_abbrev, tmv.name,  tmv.abbrev, COALESCE(tmv.subject, pm.subject) subject,y.label as year_label, ts.label as sched_label, gl.abbrev AS grade, 
...
WHERE bss.bldg_id = :bldg_id
  ...
  AND bss.school_year = :school_year 
  AND bss.seq = :seq
