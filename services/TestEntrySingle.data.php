<?php
class TestEntrySingle extends ForenaSchools {
	...
	public function studentAssessments() {
		 $xml = $this->db->query_xml('SELECT a.assessment_id, a.person_id, a.bldg_id, a.grade_level, a.test_id, a.date_taken, t.name AS test_name,
	...
	public function saveScores() {
		if (isset($_POST['to_remove'])) {
			...
			foreach ($to_remove as $assessment_id) {
				$this->db->call('a_assessment_delete(:assessment_id)', array('assessment_id' => $assessment_id));
			}
		}
	...
	public function tests() {
		return $this->db->query_xml("SELECT t.* FROM a_tests t where :grade_level BETWEEN t.min_grade AND t.max_grade AND t.allow_data_entry=true ORDER BY name", $_POST);
	}
	...
	public function buildings() {
		return $this->db->query_xml("SELECT s.school_year,s.grade_level,b.* FROM i_buildings b LEFT JOIN p_students s ON b.bldg_id = s.bldg_id AND s.person_id=:person_id
	...
	public function newAssessment() {
		return $this->db->query_xml('SELECT p.person_id, :bldg_id AS bldg_id, :grade_level AS grade_level, t.test_id, :date_taken AS date_taken, t.name AS test_name,
		  s.label AS sched_label,
